...
addpath ../../../src; % Points to MatRANS source code
GlobalVars; % Declare global variables

% MODEL INPUT ---------------------------------------------------------------

% Choose which terms to include in the model
turb = 2; % turbulence model 0: Off, 1: On (if 0 then a laminar simulation is made), 2: Transitional
...
dy0 = input_dy0; % Near-wall grid size (m)
...
if iwave == 1 % Input for second-order Stokes signal, with free stream velocity: U_1m*sin(omega*t) - U_2m*cos(2*omega*t)
  U_1m = input_U0m; % Primary amplitude of free stream velocity, U0m (m/s)
  U_2m = 0.0; % Amplitude of second harmonic free stream velocity (m/s)
elseif iwave == 2 % Input for wave form of Abreu et al. (2010)
  U_1m = input_U0m; % Total amplitude of free stream velocity signal
...
K(1:ny,1) = 1e-9.*U_1m.^2; 
W(1:ny,1) = 1e3*U_1m.^2/nu; 
...
MatRANS.h_m = h_m; MatRANS.T = T; MatRANS.U0m = U_1m;
save(OutFileName,'MatRANS'); % Save to output file
disp(''); disp(['Model results are exported to: ' OutFileName])

% Plot results from last period at omega*t=90 deg.
i90 = 1 + 72*4 + 72/4; % Time index for 90 deg. from 5th period
figure; 
subplot(1,2,1); plot(MatRANS.u(i90,:),MatRANS.y); 
xlabel('u (m/s)'); ylabel('y (m)'); % Velocity profile
title('\omegat=90^o');
subplot(1,2,2); plot(MatRANS.k(i90,:),MatRANS.y); 
xlabel('k (m^2/s^2)'); % Turbulent kinetic energy density profile