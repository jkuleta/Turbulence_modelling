...
dy0 = 7.5352e-05; % Near-wall grid size (m)
...
Uf = 0.0134; % Friction velocity (m/s)
...
% Sediment input
d = 3.0141e-06; % Grain size (m)
k_s = 7.5352e-06; % Nikuradse's equivalent sand grain roughness (m)
...
writematrix(MatRANS.u(end,:)/Uf, 'velocity.csv')
writematrix(MatRANS.k(end,:)/Uf^2, 'energy_k.csv')
writematrix(MatRANS.y/MatRANS.h_m, 'yparameter.csv')